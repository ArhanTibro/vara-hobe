...
const List = () => {
  const sliderSettings = (imageCount) => ({
    dots: imageCount > 1, // Show dots only if multiple images
    infinite: imageCount > 1, // Disable infinite loop if there's only one image
    speed: 500,
    slidesToShow: 1,
    slidesToScroll: 1,
    arrows: imageCount > 1, // Hide arrows if only one image
  });
